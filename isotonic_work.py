...
power for each contraction, integrate all positive power into isotonic work,
and return a figure plus the raw results.
"""
...
def isotonic_work_from_file(file_path: str) -> Tuple[float, float]:
    ...
    sign = np.sign(inst_power_sliced)
    crossing_idx = np.where(np.diff(sign) != 0)[0]

    # Midpoints between samples for crossing time estimates
    ...
    t_start = crossing_times[significant[0]]
    t_end = crossing_times[significant[-1] + 1]

    # Convert times to indices safely within this window
    i_start = int(np.searchsorted(time_sliced, t_start, side="left"))
    i_end = int(np.searchsorted(time_sliced, t_end, side="right"))
...
def val_isotonic_work(file_path: str,
    ...
    to validate run_isotonic_work results 
...
if st.button("Run Analysis"):
    ...
    now = datetime.now()
    st.session_state.csv_name = f"isotonic_work_{now.strftime('%Y_%m_%d_%H_%M_%S')}.csv"
            ...
            if mode == "Random Sample":
                ...
                if st.button("Run Random Sample"):
                    ...
                    for i in random_indices:
                        ...
                        fig_l, fig_f, fig_p = val_isotonic_work(val_path, i)
                        st.pyplot(fig_l)
                        st.pyplot(fig_f)
                        st.pyplot(fig_p)
